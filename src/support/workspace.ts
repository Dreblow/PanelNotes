...
export async function cleanPanelNotesConfig(): Promise<void> {
  ...
  const configUri = vscode.Uri.joinPath(vscodeDirectory, "panel-notes.json");
  ...
  for (const item of config.items) {
    ...
    const itemUri = vscode.Uri.joinPath(workspaceFolder.uri, item.path);
  ...
  const changed = validItems.length !== config.items.length;
 ...
 * Returns true only when a new item was added.
 */
export async function addMarkdownFile(uri: vscode.Uri): Promise<boolean> {
  ...
  const alreadyExists = config.items.some(
                                          (item) => normalizePath(item.path) === normalizePath(relativePath)
                                         );
 ...
 */
async function writePanelNotesConfig(config: PanelNotesConfig): Promise<void> {
  const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
